...
/**
 * Find elements by textual content. Trims whitespace but does not ignore case.
 *
 * arguments[0] {string} The exact text to match, minus leading and trailing spaces (trimmed).
 * arguments[1] {string} The selector text query.
 * arguments[2] {Element} The scope of the search.
 *
 * @return {Array.<Element>} The matching elements.
 */
clientSideScripts.findByInnerText = function() {
  var searchText = arguments[0].replace(/^\s+|\s+$/g, '');
  var selectorText = arguments[1] || '*';
  var using = arguments[2] || document;
  var elements = using.querySelectorAll(selectorText);
  var $ = typeof window.jQuery === 'function' ? window.jQuery : null;
  var matches = [];
  for (var i = 0; i < elements.length; ++i) {
    var element = elements[i];
    var elementText = ($ && $(element).text()) || element.value || element.innerText || element.textContent || element.nodeValue;
    if (elementText && (elementText.replace(/^\s+|\s+$/g, '') === searchText)) {
