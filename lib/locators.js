...
/**
 * Find an element by its text contents.
 *
 * @view
 * <button>Save</button>
 *
 * @example
 * element(by.innerText('Save'));
 * element(by.innerText('Save', 'button'));
 *
 * @param {string} searchText
 * @param {string} selectorText
 * @returns {{findElementsOverride: findElementsOverride, message: string}}
 */
MeteorStormBy.prototype.innerText = function(searchText, selectorText) {
  selectorText = selectorText || '*';
  return {
    findElementsOverride: function(driver, using) {
      return driver.findElements(
          webdriver.By.js(clientSideScripts.findByInnerText,
          searchText, selectorText, using));
    },
    message: 'by.innerText("' + searchText + '"' + (selectorText ? ', "' + selectorText + '")' : '') + ')'
  };
};
